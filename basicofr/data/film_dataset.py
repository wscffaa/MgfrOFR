...
独立于 rtn_dataset.py，支持老电影损伤退化。
基于 Film_dataset_1 的结构，使用 degradation_video_list_4。

特点：GT 和 LQ 都是灰度，用于训练灰度修复任务。

彩色版本请使用 film_dataset_color.py 中的 Film_dataset_damage_color。
...
class Film_dataset_damage(data.Dataset):
    """支持老电影损伤退化的 Dataset - 灰度版本

    特点：GT 和 LQ 都是灰度，用于训练灰度修复任务。
    使用 degradation_video_list_4 进行退化（与 Film_dataset_1 一致）。

    彩色版本请使用 Film_dataset_damage_color（在 film_dataset_color.py 中）。

    配置示例:
        type: Film_dataset_damage
    ...
    def __init__(self, data_config: dict):
        ...
        if self.is_train:
            if self.num_frame:
                ...
                logger.info(
                    f'[Film_dataset_damage] Temporal augmentation interval: [{interval_str}]; '
                    f'Random reverse: {self.random_reverse}'
                )
            else:
                logger.info('[Film_dataset_damage] Using full clips (num_frame <= 0)')
            logger.info('[Film_dataset_damage] Using grayscale degradation (degradation_video_list_4)')

        if self.normalizing:
            logger.info(f'[Film_dataset_damage] Normalization enabled')
